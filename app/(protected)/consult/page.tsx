...
function getThreadPreview(thread: ChatThread) {
  ...
  return cleanMarkdownText(latest).replace(/\s+/g, " ").slice(0, 105) || "Awaiting notes";
}
...
function renderInlineText(text: string) {
  return text.split(/(\*\*[^*]+\*\*)/g).map((part, index) => {
    ...
    if (boldMatch) {
      return (
        <strong key={`${part}-${index}`} className="font-semibold text-slate-900 dark:text-slate-50">
          {boldMatch[1]}
        </strong>
      );
    }
      ...
      {lines.map((line, index) => {
        ...
        if (orderedMatch) {
          return (
            <div
              key={`ordered-${index}`}
              className="flex items-start gap-2 rounded-lg border border-brand/10 bg-white/70 px-3 py-2 dark:border-brand/20 dark:bg-slate-900/50"
            >
              ...
              <p className="text-xs leading-5">{renderInlineText(cleanMarkdownText(orderedMatch[2] || orderedMatch[0]))}</p>
            </div>
          );
        }
        ...
        if (/^\s*[*\-•]\s+/.test(line)) {
          ...
          return (
            <div
              key={`bullet-${index}`}
              className="flex items-start gap-2 rounded-lg bg-brand/5 px-3 py-2 text-xs leading-5 dark:bg-brand/10"
            >
              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-brand" />
              <span className="text-slate-700 dark:text-slate-200">{renderInlineText(cleaned)}</span>
            </div>
          );
        }
        ...
        if (colonMatch && colonMatch[1].length <= 80) {
          return (
            <div
              ...
              className="rounded-lg border border-brand/10 bg-brand/5 px-3 py-2 dark:border-brand/30 dark:bg-brand/15"
            >
              <p className="text-[10px] font-semibold uppercase text-brand/80">
                {cleanMarkdownText(colonMatch[1])}
              </p>
              <p className="mt-1 text-xs leading-5 text-slate-700 dark:text-slate-100">
                {renderInlineText(colonMatch[2])}
              </p>
            </div>
          );
        }
        ...
        return (
          <p key={`paragraph-${index}`} className="rounded-lg bg-white/70 px-3 py-2 text-xs leading-5 text-slate-700 dark:bg-slate-900/50 dark:text-slate-200">
  ...
  return (
    <div className="space-y-2 text-sm leading-relaxed">
      {blocks.map((block, index) => {
        ...
        const firstLine = cleanMarkdownText(lines[0]);
        const headingCandidate = lines.length > 1 && !/^([*\-•]|\d+\.)/.test(lines[0]);
        ...
        if (headingCandidate) {
          ...
          return (
            <div
              key={`section-${index}`}
              className={`rounded-xl border p-3 backdrop-blur ${baseClasses}`}
            >
              <div className="flex items-center gap-2 text-sm font-semibold">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/60 text-brand dark:bg-slate-900/60">
        ...
        if (/^\d+\./.test(firstLine)) {
          return (
            <div key={`standalone-list-${index}`} className="space-y-2">
        ...
        if (/^\s*[*\-•]\s+/.test(lines[0])) {
          return (
            <div
              ...
              className="space-y-1.5 rounded-xl border border-brand/10 bg-brand/5 p-2 dark:border-brand/20 dark:bg-brand/10"
            >
              {renderSectionBody(lines)}
            </div>
          );
        ...
        return (
          <p
            key={`plain-${index}`}
            className="rounded-xl bg-white/80 px-3 py-2 text-xs leading-5 text-slate-700 shadow-inner dark:bg-slate-900/60 dark:text-slate-200"
          >
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight">AI Doctor Consultation</h1>
          <p className="mt-2 max-w-xl text-sm leading-relaxed text-slate-500 dark:text-slate-300">
        ...
        <motion.div
          className="glass-card w-full max-w-sm rounded-2xl p-3 text-sm xl:w-auto"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, delay: 0.1 }}
        >
          <div className="flex items-center gap-3">
            <Stethoscope className="h-9 w-9 rounded-xl bg-brand/10 p-2 text-brand" />
            <div>
              <p className="text-[10px] uppercase tracking-[0.18em] text-slate-400">Wellness snapshot</p>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            {consultStats.map((stat) => (
              <div key={stat.label} className="min-w-0 rounded-lg border border-white/30 bg-white/50 p-2 text-center dark:border-slate-800/60 dark:bg-slate-900/50">
                <stat.icon className="mx-auto h-3.5 w-3.5 text-brand" />
                <p className="mt-1 break-words text-sm font-semibold leading-5 text-slate-900 dark:text-slate-100">{stat.value}</p>
                <p className="mt-1 text-[9px] uppercase leading-3 text-slate-400">{stat.label}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)]">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold">Consultation history</h2>
          ...
          <div className="mt-4 space-y-3">
            {threads.length === 0 ? (
              <p className="rounded-xl border border-dashed border-slate-300/60 p-5 text-center text-xs text-slate-400">
          ...
          <div className="mt-4 rounded-2xl bg-gradient-to-br from-brand/10 via-white/60 to-brand/5 p-4 text-xs text-slate-600 shadow-inner dark:from-brand/10 dark:via-slate-900/60 dark:to-brand/10 dark:text-slate-300">
            <div className="flex items-center gap-3">
        ...
        <motion.section
          className="glass-card min-h-[500px] rounded-2xl p-5"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.4 }}
        >
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold">Live consultation</h2>
              ...
              <motion.div
                className="flex items-center gap-2 rounded-full bg-brand/10 px-3 py-1.5 text-xs font-medium text-brand"
            ...
            <div className="relative space-y-4 overflow-hidden rounded-2xl bg-gradient-to-br from-white/70 via-white to-white/70 p-4 dark:from-slate-900/60 dark:via-slate-900 dark:to-slate-900/70">
              <div className="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-brand/10 to-transparent" aria-hidden />
              {messages.length === 0 && !loading ? (
                <div className="relative z-10 flex flex-col items-center justify-center gap-3 py-12 text-center text-slate-400">
                  ...
                  {messages.map((message, index) => {
                    ...
                    if (isAssistant) {
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 space-y-2 rounded-2xl border border-brand/10 bg-white/80 p-3 shadow-lg backdrop-blur dark:border-brand/20 dark:bg-slate-900/80">
                            <div className="flex items-center justify-between gap-3">
                              <p className="text-[11px] font-semibold uppercase text-brand">Doctor&apos;s guidance</p>
                              <span className="text-[10px] uppercase tracking-wider text-slate-400">{timestamp}</span>
                            </div>
                            {renderAssistantContent(message.content)}
                            <div className="rounded-lg bg-emerald-100/60 p-2.5 text-xs text-emerald-900 dark:bg-emerald-900/30 dark:text-emerald-100">
                            ...
                            <div className="rounded-lg bg-amber-100/70 p-2.5 text-xs text-amber-900 dark:bg-amber-900/30 dark:text-amber-100">
                              <div className="flex items-center gap-2 font-semibold">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="max-w-xl space-y-2 rounded-2xl bg-brand text-left text-white shadow-lg">
                          <div className="flex items-center justify-between gap-3 rounded-t-2xl bg-brand/80 px-4 py-2.5 text-[10px] uppercase tracking-[0.18em]">
                            <span>Patient briefing</span>
                            <span className="text-white/70">{timestamp}</span>
                          </div>
                          <p className="px-4 pb-3 text-sm leading-relaxed">{message.content}</p>
                          {message.imageBase64 && (
            ...
            <div className="space-y-3 rounded-2xl border border-white/40 bg-white/80 p-4 backdrop-blur dark:border-slate-800/60 dark:bg-slate-900/70">
              <Textarea
                value={input}
                onChange={(event) => setInput(event.target.value)}
                placeholder="Describe your symptoms, recent test results, or daily wellness notes."
                className="min-h-[110px] rounded-xl border-none bg-white/60 text-sm shadow-inner focus-visible:ring-2 focus-visible:ring-brand dark:bg-slate-900/60"
              />
            ...
            <div className="rounded-2xl bg-gradient-to-br from-brand/5 via-white/60 to-brand/10 p-4 text-xs text-slate-500 shadow-inner dark:from-brand/10 dark:via-slate-900/60 dark:to-brand/20 dark:text-slate-300">
              <p className="font-semibold uppercase tracking-[0.22em] text-brand">Medical caution</p>
              <p className="mt-2 leading-relaxed">
